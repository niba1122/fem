...
subroutine ebe_scg(u,model,bc,f)
	...
	type(struct_bc) :: bc
	real(8) :: u(:),f(:),a,b
	real(8),allocatable :: s(:),Ke(:,:),r0(:),r1(:),p(:),Ku0(:),kp(:), Kall(:,:),norm_r0,sr0(:),sr1(:)
	...
	allocate(sr0(n_nds*dim),sr1(n_nds*dim))
...
! if (step == 1) print *,kall
		...
		a = dot_product(r0,sr0)/dot_product(p,kp)
...
! print *,"alpha = ",a
		...
		b = dot_product(r1,sr1)/dot_product(r0,sr0)
