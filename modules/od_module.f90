!
! module for treat orthotropic damage
! 
...
subroutine od_calc_angle(angle,model,n_mat)
	type(struct_model) :: model
	integer i,n_mat,n_els
	double precision,pointer,dimension(:,:) :: nodes
	integer,pointer,dimension(:,:) :: elements
...
function od_calc_BDB(model,i_el,coord,data)
	type(struct_model) :: model
	integer :: i_el
...
subroutine od_calc_output(output,model,angle,u,D_)
	type(struct_model) :: model
